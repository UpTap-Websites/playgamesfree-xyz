  ...
  for (const item of categories) {
    const tmp = await fetchAPI(
      ...
      query ($category: String, $limit: Int) {
        games (filter: { category: { name: { _eq: $category } } }, limit: $limit, sort: "-featured") {
          title
          slug
          gid
          featured
...
// 全部游戏
export const getDataForAll = async (limit = PER_PAGE) => {
  // 获取分类列表
  const categories = await getCategories();
...
export const getGamesByCategorySlug = async (slug, limit = PER_PAGE) => {
...
export const getGameBySlug = async (slug, limit = 24) => {
  const data = await fetchAPI(
    ...
    query ($slug: String, $limit: Int) {
      ...
      related: games ( filter: { slug: { _neq: $slug } }, limit: $limit, sort: "-featured" ) {
        title
        slug
        gid
      }
